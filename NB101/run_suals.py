...
from suals.suals_rg import LocalSearch
...
ls = LocalSearch(cs=cs, dimensions=dimensions, f=f, configspace=cs, budget=max_budget, b=b, output_path=output_path)
...
for run_id, _ in enumerate(range(args.runs), start=args.run_start):
    ...
    # Running LS iterations
    if run_id == 0:
        runtime = ls.run(generations=args.gens, verbose=args.verbose, seed=run_id)
    else:
        runtime = ls.run(generations=args.gens, verbose=args.verbose, seed=run_id)
